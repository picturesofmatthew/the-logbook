...
- **Current focus (2026-07-23, on `main`):** the **Lighthouse world IS the app** — `/` renders the world; opening the site crosses a **mandatory cold-open gate** (whole world → tap *begin* → a fluid push-in to the hearth). All five rooms (hearth · garden · docks · library · lantern) are real and **swipe/rise-navigable**; the day is logged at the hearth; the old glade home retired (its glade → the Garden room, its seal → the hearth mantle, its horizon → the Docks). Shipped as **World Engine 1–9/n**, live on `main`. Now running the **review + simplify pass** (`REVIEW-PLAN.md`): the core loop is **verified moved intact** (byte-identical), a11y/interaction fixes + an **in-world invite** landing, then a `/simplify` pass. 75/75 tests.
- **⭐ NEXT — polish the world + ornate graphics.** The world's *structure* is built (rooms, swipe/rise, gate, lantern-lit-by-spell); what remains: (1) the **ornate hero graphics** — a **Fable-subagent plan** for sigil-level art (the hearth-hall mantle/cast with two chosen sprites + splayed Spellbook, the tower, the island) → polish passes; today's art is v1 CSS/SVG. (2) **In-world book/vessel interiors** — books/vessel currently route OUT to sub-pages (`/book`, `/trends`, `/library`, `/shore`), breaking immersion; make them in-world spreads. (3) **Settings/invite fully in-world.** (4) The **cast cinematics** — the cast rises to the lamp → beam sweeps the shore → ceremonious return. Discipline unchanged: game-*feel* not scope; **current web stack** (NOT the parked native rewrite); spectacle stays *earned*.
