...
- **⭐ Current focus (2026-07-24 night, branch `feat/world-interactivity` — NOT merged, NOT deployed):** **the atoms are built.** Two exploration lenses converged (over-engineering audit + fresh-angles pass): retention and the moat live in **two atoms — the light that waits for your partner, and the record you'd never delete** — not in the five-room world. So the world is **frozen**, the daily loop shrank to a light and a word, and the free cuts were taken. Shipped tonight per `THE-ATOMS-HANDOFF.md` (5/5): **the Carry** (the first chord that sounds on *difference* — one keeper low, the other strong; 13 chords now) · **the Sealed Word** (a line to your keeper, encrypted at rest, opened only when the ring closes, kept forever in the book — the reveal law is one pure tested module) · **two free cuts** (`world-spread` title-page stub deleted, hotspots open the real pages; the bestiary derives `arrivedOn` instead of writing at render) · **the Letter** (write a line + press the seal → `/join` unfurls with the living half-lit seal; signup now elects a **character** + **vow**; a once-per-book arrival greeting) · **the patient day** (the couple-day turns at a **4am grace hour**, `COUPLE_DAY_ROLLOVER`, one chokepoint, unit-tested — and the hearth says it out loud past midnight). **89/89 · tsc clean · build green.** Migration `0011` generated **and applied** (additive/nullable). **Next: dogfood the two atoms before adding anything**; then the v1.1 crossing-at-the-×, the elected character at the hearth mantle, and the leaner daily face (**Matthew's call** — a redesign, not a unilateral one). *Previous focus (07-24 day): catalogued the world-interactivity arc.*
- **Superseded context (kept for the map):** **catalogued the next build arc — full world interactivity.** This session wrote `WIRE-THE-WORLD-HANDOFF.md` (three initiatives, speced to real code seams) + new social canon `THE-BEACON.md`, and locked three calls: food alternatives = **pure deterministic** (same hall, lower-cal/higher-protein, no seed/no LLM); character = **6–8 Inklight keeper archetypes** (fulfils the canon "each keeper elects their own sprite at the mantle"); the partner invite = a **$0 share-link Letter** (the craft is in the *unfurl*, not the transport). Key reframe: **the rooms are already live (data-bound), not shells — the work is adding *verbs*, not wiring.** Last shipped (07-23): **all four rooms as their own worlds** — the Lighthouse world IS the app (`/`, crossed through the cold-open gate), and each room got its ornate hero art (Fable proto → port) + in-world interiors. Shipped + deployed as **World Engine 12–14/n**: the **Library** (ornate Compendium + the in-world **book spread**), the **Lantern** (three live watches: lit/kindled/dark + the beam), the **Docks** (the vessel/score + the two roads + the **shore interior**), the **Garden** (ornate setting wrapped around the living GladeScene). Interiors open **in place** (never route out) via `components/world/world-spread.tsx`. Pipeline: **Fable proto → port subagent (self-verifying) → I own the shared architecture + review + commit.** 75/75 · build green.
- **NEXT (pre-atoms plan, now partly superseded — the world half is FROZEN). (1) BUILD: wire the world's verbs per `WIRE-THE-WORLD-HANDOFF.md`** — onboarding-as-cold-open + the beautiful Letter (**land before first signup** so Matthew + Kennedy's profiles carry character + vow), the Apothecary's Counsel (deterministic food alternatives, engine-first), and Beacon guardrails (`THE-BEACON.md` — keep the beam aimable, the coffer the pledge rail; the social feature itself is future). **(2) POLISH: the cohesive visual audit (see `WORLD-AUDIT-HANDOFF.md`).** Matthew's call: take all four rooms in **together as one world of logic** — assess perspective, cross-room consistency (shared sky, travelling light, the far shore, the gold/silver grammar, relational violet, the one key), and the **emotional value of details** per screen. Matthew provides screenshots; audit **imagery + codebase in unison**. Then iterate live (cheap — DOM/SVG hot-reload). Also queued: **deepen the interiors** — the **Book of Days** into the true rereadable day-by-day spread (real sigils, both logs facing), the shore interior likewise (grow both in `world-spread.tsx`); the **cast cinematic** (rise to the lamp → beam sweeps the shore); settings/invite fully in-world. Discipline unchanged: game-*feel* not scope; current web stack; spectacle stays *earned*; every ornament on a true number.
...
- The **Apothecary** (the museum / `foods` table) IS the food database; USDA is only touched when donating a new specimen
...
- The shared "today" — the `day` key for every log + the seal — buckets on one **couple timezone** (`COUPLE_TZ` env → `coupleTz()` in `lib/dates.ts`), so two devices in two zones can't split the seal; the device-tz cookie (`components/tz-sync.tsx`) is now **display-only**. ⚠ set `COUPLE_TZ` in Vercel prod (falls back to Denver). **The day is now PATIENT (2026-07-24):** it turns at a grace hour after midnight, not at midnight — default **4am couple-time**, `COUPLE_DAY_ROLLOVER` env to change it (0 = strict midnight; clamped 0–8). One chokepoint: `coupleDayOf()` → `coupleDayFor()` in `lib/dates-client.ts`, unit-tested; the one-key invariant is unchanged, only where that one clock's day begins. The world says so out loud past midnight (`inNightGrace()` → the hearth's standing line). Still same-couple-day until the deeper async close window (36–48h) lands.
