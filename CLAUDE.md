...
- **Current focus:** "Inklight" overhaul (spec: `DIRECTION.md` v3). **Phases 0–4 ALL SHIPPED to prod** (2026-07-19): light script · engines (27 tests) · Training Log · DaySeal · Spellbook (`/book`) · the Glade scene · sound pass (`legendaryTone`/`arrivalTone`/`inscribeTick`) · being-arrival ceremonies (`being_arrivals` table, stag/heron only — others arrive silently until they have art) · enter page rebranded ("sign×sealed, a spellbook for two"). The overhaul core is COMPLETE. Next candidates: masters from `art/prompts.md` (upgrade scene/fox/sigil parts), remaining beings' art + their arrival ceremonies, trends-as-book-plate, real passcodes + FDC key, seasonal October content, Phase 5 launch track (Expo, from Chiang Mai)
