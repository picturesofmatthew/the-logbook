...
// ── THE LIBRARY — above, the Compendium: warm shelves, the five books ──
export function LibraryStub() {
  const spineH = [220, 268, 300, 262, 210];
  return (
    <>
      <defs>
        <linearGradient id="lb_wall" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stopColor="#3a2c1e" />
          <stop offset="1" stopColor="#281e14" />
        </linearGradient>
        <radialGradient id="lb_lamp" cx="0.5" cy="0.44" r="0.55">
          <stop offset="0" stopColor="#f0cb7e" stopOpacity="0.5" />
          <stop offset="1" stopColor="#f0cb7e" stopOpacity="0" />
        </radialGradient>
      </defs>
      <rect x="0" y="0" width="1000" height="1500" fill="url(#lb_wall)" />
      <ellipse cx="500" cy="700" rx="470" ry="470" fill="url(#lb_lamp)" />
      {/* shelves down both walls */}
      <g fill="#20180f" opacity="0.9">
        {[300, 470, 640, 810].map((y) => (
          <g key={`l${y}`}>
            <rect x="40" y={y} width="150" height="128" rx="4" />
            <rect x="810" y={y} width="150" height="128" rx="4" />
          </g>
        ))}
      </g>
      <g fill="#7a5a34" opacity="0.7">
        {[300, 470, 640, 810].map((y) =>
          [56, 84, 112, 140, 168].map((x) => (
            <g key={`${y}-${x}`}>
              <rect x={x} y={y + 14} width="18" height="100" rx="2" />
              <rect x={x + 754} y={y + 14} width="18" height="100" rx="2" />
            </g>
          )),
        )}
      </g>
      {/* the five books, standing on a central lectern */}
      <rect x="286" y="984" width="428" height="26" rx="6" fill="#33261a" />
      <g>
        {spineH.map((h, i) => {
          const x = 320 + i * 74;
          const gold = i === 2;
          return (
            <g key={i}>
              <rect
                ...
                height={h}
                rx="5"
                fill={gold ? "#caa25a" : "#5a4126"}
                stroke="#e8c886"
                strokeWidth="2.5"
                opacity={gold ? 0.95 : 0.82}
              />
              <rect x={x + 10} y={984 - h + 26} width="36" height="8" rx="3" fill="#e8c886" opacity="0.55" />
            </g>
          );
        })}
      </g>
      {/* the spiral stair, arriving from below */}
      <g stroke="#3a2c1c" strokeWidth="12" fill="none" opacity="0.7" strokeLinecap="round">
